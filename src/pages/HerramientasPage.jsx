...
function FilaComparativa({ label, s, l, hi }) {
  const fmtCelda = (v) => {
    if (v === 0 || v === undefined) return { txt: '0', color: 'var(--accent)' }
    const signo = v > 0 ? '+ ' : '− '
  ...
  return (
    <div className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        <div>
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Tipo de contrato</label>
        ...
        <div>
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>{tipo === 'servicios' ? 'Honorario' : 'Salario'} mensual</label>
        ...
        <div className="col-span-2">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Clase de riesgo ARL</label>
      ...
      <button onClick={calcular} className="w-full text-white font-bold py-2 rounded-xl transition text-sm" style={{ background: 'var(--accent-deep)' }}>Calcular equivalencia</button>
      {res && (
        <div className="space-y-3">
          <div className="rounded-xl border border-blue-100 overflow-hidden">
            <div className="grid gap-1 px-3 py-2" style={{ gridTemplateColumns: '1.3fr 1fr 1fr', background: 'var(--surface-raised)' }}>
              <span className="text-[10px] font-bold" style={{ color: 'var(--text-tertiary)' }}>Concepto</span>
              <span className="text-[10px] font-bold text-right" style={{ color: 'var(--accent-deep)' }}>Honorario</span>
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-xl border border-blue-100 overflow-hidden">
        <table className="w-full">
          <thead style={{ background: 'var(--surface-raised)' }}><tr><th className="py-1.5 px-3 text-left text-xs font-bold" style={{ color: 'var(--text-tertiary)' }}>Mes</th><th className="py-1.5 px-3 text-right text-xs font-bold" style={{ color: '#0369a1' }}>Honorario</th></tr></thead>
          <tbody>
            {MESES.map((m, i) => (
              <tr key={m} className="border-t border-blue-50">
                <td className="py-1 px-3 text-xs" style={{ color: 'var(--accent-deep)' }}>{m}</td>
      ...
      <button onClick={calcular} className="w-full text-white font-bold py-2 rounded-xl transition text-sm" style={{ background: 'var(--accent-deep)' }}>Calcular</button>
      {res && (
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-2">
...
function CalcDiluciones() {
  ...
  return (
    <div className="space-y-3">
      <div>
        <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>¿Qué quiero calcular?</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        {['c1','v1','c2','v2'].map(k => (
          <div key={k} className={inc === k ? 'opacity-40 pointer-events-none' : ''}>
            <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>{labels[k]}</label>
      ...
      <button onClick={calcular} className="w-full text-white font-bold py-2 rounded-xl text-sm" style={{ background: 'var(--accent-deep)' }}>Calcular (C₁V₁ = C₂V₂)</button>
      {res && (
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2 items-end">
        <div className="flex-1">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Valor a convertir</label>
          <input value={valor} onChange={e => setValor(e.target.value)} placeholder="0" inputMode="decimal"
            className="w-full border border-blue-100 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-blue-400" />
        </div>
        <div className="flex-1">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Unidad de entrada</label>
      ...
      <div className="flex gap-2">
        <div className="flex-1">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--text-tertiary)' }}>Densidad (g/mL) <span className="font-normal">— % v/v</span></label>
          <input value={densidad} onChange={e => setDensidad(e.target.value)} placeholder="1.00" inputMode="decimal"
            className="w-full border border-blue-100 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-blue-400" />
        </div>
        <div className="flex-1">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--text-tertiary)' }}>PM (g/mol) <span className="font-normal">— molar</span></label>
      ...
      <div className="rounded-2xl border border-blue-100 overflow-hidden" style={{ background: '#fff' }}>
        <div className="px-3 py-2" style={{ background: 'var(--surface-raised)', borderBottom: '1px solid var(--border)' }}>
          <p className="text-xs font-bold" style={{ color: 'var(--accent-deep)' }}>Equivalencias</p>
        </div>
        {res ? Object.entries(res).map(([k, v]) => (
          <div key={k} className="flex justify-between items-center px-3 py-2 border-t border-blue-50 first:border-0">
            ...
            <span className="text-sm font-bold tabular-nums" style={{ color: 'var(--accent-deep)' }}>{v}</span>
          </div>
        )) : (
          <div className="px-3 py-4 text-center text-xs" style={{ color: 'var(--text-tertiary)' }}>Ingresa un valor para ver las equivalencias</div>
        )}
      </div>
    </div>
  )
...
function CalcPureza() {
  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-2">
        <div className="flex-1">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Masa del reactivo (g)</label>
          <input value={masa} onChange={e => setMasa(e.target.value)} placeholder="100" inputMode="decimal"
            className="w-full border border-blue-100 rounded-xl px-3 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-blue-400" />
        </div>
        <div className="flex-1">
          <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Pureza del certificado (%)</label>
      ...
      <div className="grid grid-cols-3 gap-2">
        {[
          ['Masa total', res ? fmt(res.m) : '—', 'var(--surface-raised)', 'var(--accent-deep)'],
...
function CalcPH() {
  ...
  const pHColor = pHNum < 4 ? '#dc2626' : pHNum < 6.5 ? '#ea580c' : pHNum < 7.5 ? '#16a34a' : pHNum < 10 ? 'var(--accent-deep)' : '#7c3aed'

  return (
    <div className="space-y-3">
      <div>
        <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Tipo de cálculo</label>
      ...
      <div className="grid grid-cols-2 gap-2">
        {(tipo !== 'buffer') && (
          <div className="col-span-2">
            <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>Concentración (mol/L)</label>
        ...
        {(tipo === 'debil' || tipo === 'buffer') && (
          <div className={tipo === 'buffer' ? '' : 'col-span-2'}>
            <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>pKa del ácido</label>
        ...
        {tipo === 'buffer' && (<>
          <div>
            <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>[Ácido] (mol/L)</label>
          ...
          <div>
            <label className="block text-xs font-semibold mb-0.5" style={{ color: 'var(--accent-deep)' }}>[Base conj.] (mol/L)</label>
      ...
      <button onClick={calcular} className="w-full text-white font-bold py-2 rounded-xl text-sm" style={{ background: 'var(--accent-deep)' }}>Calcular pH</button>
      {res && (
...
function FormulacionSimple() {
  ...
  return (
    <div className="space-y-4">
      {/* Ingredientes */}
      <div>
        <p className="text-xs font-bold mb-1.5" style={{ color: 'var(--accent-deep)' }}>Ingredientes</p>
        <div className="rounded-xl overflow-hidden border border-blue-100">
          <div className="grid grid-cols-12 gap-1 px-2 py-1.5 text-[10px] font-bold" style={{ background: 'var(--surface-raised)', color: 'var(--text-tertiary)' }}>
        ...
        <button onClick={addIng} className="mt-1.5 w-full flex items-center justify-center gap-1.5 py-1.5 rounded-xl text-xs font-semibold border-2 border-dashed border-blue-200 transition-colors hover:bg-blue-50" style={{ color: 'var(--accent-deep)' }}>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <p className="text-xs font-bold" style={{ color: 'var(--accent-deep)' }}>Presentaciones</p>
          <div className="flex items-center gap-1.5">
            <label className="text-[10px]" style={{ color: 'var(--text-tertiary)' }}>Margen %</label>
        ...
        <div className="rounded-xl overflow-hidden border border-blue-100">
          <div className="grid grid-cols-10 gap-1 px-2 py-1.5 text-[10px] font-bold" style={{ background: 'var(--surface-raised)', color: 'var(--text-tertiary)' }}>
        ...
        <button onClick={addPres} className="mt-1.5 w-full flex items-center justify-center gap-1.5 py-1.5 rounded-xl text-xs font-semibold border-2 border-dashed border-blue-200 hover:bg-blue-50" style={{ color: 'var(--accent-deep)' }}>
          <Plus size={13} /> Agregar presentación
        </button>
      </div>

      <button onClick={calcular} className="w-full text-white font-bold py-2 rounded-xl text-sm" style={{ background: 'var(--accent-deep)' }}>Calcular costos</button>
      ...
      {res && (
        <div className="space-y-3">
          ...
          {/* Tabla ingredientes */}
          <div className="rounded-xl overflow-hidden border border-blue-100">
            <div className="grid grid-cols-12 px-2 py-1.5 text-[10px] font-bold" style={{ background: 'var(--surface-raised)', color: 'var(--text-tertiary)' }}>
              <span className="col-span-4">Ingrediente</span>
              <span className="col-span-3 text-right">% p/p</span>
            ...
            {res.filas.map((f, i) => (
              <div key={i} className="grid grid-cols-12 px-2 py-1.5 border-t border-blue-50">
                <span className="col-span-4 text-xs" style={{ color: 'var(--accent-deep)' }}>{f.nombre || `Ing. ${i + 1}`}</span>
                <span className="col-span-3 text-right text-xs tabular-nums" style={{ color: 'var(--text-tertiary)' }}>{fmt(f.pct * 100, 1)}%</span>
                <span className="col-span-5 text-right text-xs font-semibold tabular-nums" style={{ color: 'var(--accent-deep)' }}>{cop(f.aporte)}</span>
              </div>
            ))}
          </div>
          ...
          <div className="rounded-xl overflow-hidden border border-blue-100">
            ...
            {res.pres.map((p, i) => (
              <div key={i} className="grid grid-cols-12 px-2 py-1.5 border-t border-blue-50">
                <span className="col-span-3 text-xs font-semibold" style={{ color: 'var(--accent-deep)' }}>{p.label || `${p.g}g`}</span>
                <span className="col-span-2 text-right text-xs tabular-nums" style={{ color: 'var(--text-tertiary)' }}>{fmt(numF(p.g), 0)}</span>
                <span className="col-span-4 text-right text-xs font-bold tabular-nums" style={{ color: 'var(--accent-deep)' }}>{cop(p.costoMP)}</span>
...
function FAQ({ pregunta, respuesta }) {
  ...
  return (
    <div className="border border-blue-100 rounded-xl overflow-hidden">
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-3 py-2.5 text-left text-xs font-semibold hover:bg-blue-50 transition" style={{ color: 'var(--accent-deep)' }}>
        {pregunta}
        {open ? <ChevronUp size={14} /> : <ChevronDown size={14} />}
      </button>
      {open && <p className="px-3 pb-3 text-xs leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>{respuesta}</p>}
    </div>
  )
}
...
export default function HerramientasPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-3 py-4 space-y-4">
      ...
      {/* Card calculadora */}
      <div className="rounded-2xl overflow-hidden shadow-sm" style={{ background: '#ffffff', border: '1px solid var(--border)' }}>

        {/* Sub-tabs */}
        <div className="flex p-2 gap-1.5" style={{ background: 'var(--surface-raised)', borderBottom: '0.5px solid var(--border)' }}>
          {TABS_LAB.map(t => (
        ...
        <div className="px-3 pt-3 pb-1">
          <h2 className="font-bold text-sm" style={{ color: 'var(--accent-deep)' }}>{titulo[tabLab]}</h2>
        </div>
      ...
      {/* Info */}
      <div className="rounded-xl p-3 flex gap-2" style={{ background: 'rgba(239,246,255,0.8)', border: '0.5px solid var(--border)' }}>
        <Info size={13} style={{ color: 'var(--accent-deep)', flexShrink: 0, marginTop: 1 }} />
        <p className="text-[11px] leading-relaxed" style={{ color: 'var(--text-tertiary)' }}>
          <strong style={{ color: 'var(--accent-deep)' }}>Laboral:</strong> SMMLV 2026 $1.750.905 (Decreto 0159).
        </p>
      </div>
