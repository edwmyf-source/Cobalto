...
const WIDGET_FORM_DEFAULT = { titulo: '', imagen_url: '', imagen_emoji: '🧪', imagen_gradient: 'linear-gradient(135deg,var(--accent-deep),var(--accent))', btn_url: '', btn_texto: 'Más información', btn_color: '#FFFFFF', btn_text_color: 'var(--accent-deep)', orden: 0, activo: true }
            ...
            <div className="flex gap-2 flex-wrap">
              {GRADIENTS.map(g => (
                <button key={g} onClick={() => setForm(f => ({...f, imagen_gradient: g}))}
                  ...
                  style={{ background: g, borderColor: form.imagen_gradient === g ? 'var(--accent-deep)' : 'transparent' }} />
              ))}
            </div>
          </div>
          <div>
