    ...
    <div className="page-enter max-w-2xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <h2 className="font-bold text-xl" style={{ color: '#5C1A2E' }}>Notificaciones</h2>
          {unreadCount > 0 && (
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl p-10 text-center" style={{ border: '1px solid #EEE3E5' }}>
          <Bell size={32} className="mx-auto mb-3" style={{ color: '#E0CFD3' }} />
          <p className="text-sm font-medium" style={{ color: '#B09499' }}>
            Aún no tienes notificaciones de interacciones.
          </p>
          <p className="text-xs mt-1" style={{ color: '#D0AEB4' }}>
                  ...
                  return (
                    <button key={n.id} onClick={() => handleOpen(n)}
                      ...
                      style={{
                        ...
                        borderLeft: isUnread ? '4px solid #5C1A2E' : '1px solid #EEE3E5',
                      }}>

                      <div className="flex items-center gap-3 px-3 py-3">
                        {/* Avatar con iniciales */}
                        <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 text-[13px] font-bold"
                          style={{ background: isUnread ? '#5C1A2E' : '#F5EFF1', color: isUnread ? '#fff' : '#B09499' }}>
                          {getInitials(senderName)}
                        </div>

                        {/* Nombre + accion + tiempo */}
                        <div className="flex-1 min-w-0">
                          <div className="text-[14px] leading-snug" style={{ color: '#5C1A2E' }}>
                            <span className="font-semibold">{senderName || 'Usuario'}</span>
                            {action && (
                              <span style={{ color: '#B09499', fontWeight: 400 }}> {action}</span>
                            )}
                          </div>
                          <div className="text-[11px] mt-0.5" style={{ color: '#D0AEB4' }}>
                        ...
                        {isUnread && (
                          <div className="w-2.5 h-2.5 rounded-full flex-shrink-0"
                            style={{ background: '#2F5233', boxShadow: '0 0 8px rgba(255,183,3,0.65)' }} />
                        )}
                      </div>
                      ...
                      {snippet && (
                        <div className="mx-3 mb-3 px-3 py-2 rounded-lg text-[12px] leading-relaxed"
                          style={{ background: '#FAF7F5', borderLeft: '3px solid #E0CFD3', color: '#B09499' }}>
