      ...
      <div className="max-w-5xl mx-auto h-14 flex items-center gap-4 px-4">
        ...
        <button onClick={() => navigate('/feed')} className="flex items-center gap-2 flex-shrink-0">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: '#0047AB' }}>
            <FlaskConical size={16} color="#6B87B8" />
          </div>
          <span className="text-base font-bold hidden lg:block" style={{ color: '#0047AB', letterSpacing: '.5px' }}>CeQu.com</span>
        </button>

        {/* Buscador */}
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg flex-1 max-w-xs"
          style={{ background: '#FFFFFF', border: '1px solid #D6E2F5' }}>
          <Search size={14} color="#6B87B8" />
          <span className="text-xs" style={{ color: '#2C6BD4' }}>Buscar en CeQu...</span>
        </div>
            ...
            return (
              <button key={id} onClick={() => navigate(id)}
                ...
                <div className="relative">
                  <Icon size={18} color={active ? '#0047AB' : '#6B87B8'} />
                  {badge > 0 && (
                ...
                <span className="text-[10px] font-medium" style={{ color: active ? '#0047AB' : '#6B87B8' }}>
                  {label}
                </span>
              </button>
            )
        ...
        {/* Divider */}
        <div className="w-px h-7 flex-shrink-0" style={{ background: '#D6E2F5' }} />
        ...
        <div className="relative flex-shrink-0" ref={menuRef}>
          <button onClick={() => setMenuOpen(o => !o)}
            ...
            <span className="text-[10px] font-medium flex items-center gap-0.5" style={{ color: '#2C6BD4' }}>
          ...
          {menuOpen && (
            <div className="absolute right-0 top-full mt-1 rounded-xl overflow-hidden z-50"
              style={{ background: '#fff', border: '1px solid #D6E2F5', boxShadow: '0 8px 32px rgba(13,27,62,0.18)', minWidth: 200 }}>
              <div className="px-4 py-3 border-b" style={{ borderColor: '#D6E2F5' }}>
                <p className="text-sm font-bold" style={{ color: '#0047AB' }}>{name}</p>
                <p className="text-xs" style={{ color: '#2C6BD4' }}>{session?.user?.email}</p>
              </div>
              ...
              <div style={{ borderTop: '1px solid #D6E2F5' }}>
