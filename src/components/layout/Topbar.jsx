...
export default function Topbar({ title, profile, unreadCount = 0 }) {
  ...
  return (
    <header className="sticky top-0 z-30 h-14 flex items-center justify-between px-4 md:px-8"
      style={{ background: 'rgba(255,255,255,0.65)', backdropFilter: 'blur(16px)', borderBottom: '0.5px solid rgba(147,197,253,0.4)' }}>
      <h1 key={title} className="slide-in-left font-bold text-base" style={{ color: '#1e3a5f' }}>{title}</h1>
      <div className="flex items-center gap-3">
        ...
        <span className="text-sm font-medium hidden sm:block" style={{ color: '#1e3a5f' }}>{profile?.full_name}</span>
