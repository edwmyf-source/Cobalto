...
export default function Topbar({ profile, unreadCount = 0, session }) {
  ...
  return (
    <header className="sticky top-0 z-40 w-full"
      ...
      <div className="max-w-[1400px] mx-auto h-14 flex items-center gap-4 px-4 md:px-10">
        ...
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg flex-1 max-w-xs min-w-0"
          style={{ background: 'var(--bg-app)', border: '1px solid var(--border-strong, #C7D2E4)' }}>
          <Search size={15} className="flex-shrink-0" style={{ color: "var(--accent-deep)" }} />
          <span className="text-xs truncate" style={{ color: 'var(--text-secondary)' }}>Buscar en Red Cobalto...</span>
        </div>
          ...
          {navItems.map(({ id, label, Icon, badge }) => {
            ...
            return (
              <button key={id} onClick={() => navigate(id)}
                ...
                style={{ borderBottomColor: active ? 'var(--accent-deep)' : 'transparent', minWidth: 56 }}>
                <div className="relative">
                  <Icon size={18} color={active ? 'var(--accent-deep)' : 'var(--text-tertiary)'} />
                  {badge > 0 && (
                ...
                <span className="text-[10px] font-medium" style={{ color: active ? 'var(--accent-deep)' : 'var(--text-tertiary)' }}>
                  {label}
                </span>
              </button>
            )
        ...
        <button onClick={() => navigate('/feed')}
          ...
          style={{ background: '#fff', border: '1.5px solid var(--accent-deep)', color: 'var(--accent-deep)' }}>
          <Pencil size={13} />
          Publicar
        </button>
