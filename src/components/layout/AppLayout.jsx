  ...
  return (
    <div className="min-h-screen" style={{ minHeight: '100vh', background: '#FFFFFF' }}>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 flex items-center px-[18px] h-14"
        style={{ background: 'radial-gradient(circle at 70% -50%, #1A5AC8 0%, #0B2E68 55%, #081F4A 100%)' }}>
        <span className="font-extrabold text-[19px]" style={{ color: '#ffffff', letterSpacing: '-0.035em' }}>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40" style={{ pointerEvents: 'none' }}>
        ...
        <div className="mx-4 mb-4 h-[60px] rounded-[24px] flex items-center justify-around px-2"
          style={{ background: '#FFFFFF', boxShadow: '0 14px 40px rgba(8,31,74,0.24)', pointerEvents: 'auto' }}>
          ...
          {(() => { const active = currentTab === '/feed'; return (
            <button onClick={() => navigate('/feed')}
              ...
              <span className="text-[11px] font-extrabold" style={{ color: active ? '#0B2E68' : '#8FA3C7' }}>Feed</span>
              {active && <div style={{ width: 14, height: 2.5, borderRadius: 2, background: 'linear-gradient(90deg,#0B2E68,#1A5AC8)' }} />}
            </button>
          )})()}
          ...
          {(() => { const active = currentTab === '/chats'; return (
            <button onClick={() => navigate('/chats')}
              ...
              <div className="relative">
                <span className="text-[11px] font-extrabold" style={{ color: active ? '#0B2E68' : '#8FA3C7' }}>Mensajes</span>
                {unreadCount > 0 && (
          ...
          <div className="flex-shrink-0 px-1" style={{ marginTop: -22 }}>
            <button onClick={() => navigate('/feed?publish=1')} aria-label="Publicar"
              ...
              style={{ background: 'linear-gradient(135deg,#0B2E68,#1A5AC8)', boxShadow: '0 8px 24px rgba(11,46,104,0.45), inset 0 1px 0 rgba(255,255,255,0.25)', border: '3px solid #E4EBF7' }}>
          ...
          {(() => { const active = currentTab === '/feed' && searchParams.get('buscar') === '1'; return (
            <button onClick={() => navigate('/feed?buscar=1')}
              ...
              <span className="text-[11px] font-extrabold" style={{ color: active ? '#0B2E68' : '#8FA3C7' }}>Buscar</span>
              {active && <div style={{ width: 14, height: 2.5, borderRadius: 2, background: 'linear-gradient(90deg,#0B2E68,#1A5AC8)' }} />}
            </button>
          )})()}
          ...
          {(() => { const active = currentTab === '/contacts'; return (
            <button onClick={() => navigate('/contacts')}
              ...
              <span className="text-[11px] font-extrabold" style={{ color: active ? '#0B2E68' : '#8FA3C7' }}>Personas</span>
              {active && <div style={{ width: 14, height: 2.5, borderRadius: 2, background: 'linear-gradient(90deg,#0B2E68,#1A5AC8)' }} />}
            </button>
          )})()}
          ...
          <button ref={profileBtnRef} onClick={() => setProfileMenuOpen(o => !o)}
            ...
            <span className="text-[11px] font-extrabold" style={{ color: profileMenuOpen ? '#0B2E68' : '#8FA3C7' }}>Perfil</span>
            {profileMenuOpen && <div style={{ width: 14, height: 2.5, borderRadius: 2, background: 'linear-gradient(90deg,#0B2E68,#1A5AC8)' }} />}
          </button>
