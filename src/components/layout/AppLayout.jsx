...
export default function AppLayout() {
  ...
  return (
    <div className="flex min-h-screen" style={{minHeight:"100vh"}}>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 flex items-end" style={{background:"rgba(255,255,255,0.75)",backdropFilter:"blur(16px)",borderTop:"0.5px solid rgba(147,197,253,0.4)"}}>
            ...
            <Fragment key={item.id}>
              {insertPlus && (
                <button key="fab-plus" onClick={() => navigate('/feed?publish=1')}
                  ...
                  <span className="w-[52px] h-[52px] rounded-full active:scale-95 transition-all flex items-center justify-center border-4 border-white -mb-1" style={{background:"#2563eb",boxShadow:"0 4px 16px rgba(37,99,235,0.35)"}}>
              ...
              <button onClick={() => navigate(item.id)}
                ...
                <div className="relative">
                  <Icon size={26} style={{color: active ? '#2563eb' : '#93c5fd'}} />
                  {item.badge > 0 && (
