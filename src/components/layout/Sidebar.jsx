...
import { LayoutList, MessageSquare, User, HelpCircle, Lock, LogOut, Plus, Bell, Calendar, Mail } from 'lucide-react'
...
export default function Sidebar({ currentPath, navigate, profile, unreadCount = 0 }) {
  ...
  return (
    <aside className="fixed left-0 top-0 h-screen w-[220px] flex flex-col z-40"
      style={{ background: 'linear-gradient(180deg, #182D4F 0%, #10213A 100%)', borderRight: '1px solid rgba(255,255,255,0.08)' }}>
      ...
      <div className="px-3 mb-4">
        <button onClick={() => navigate('/feed?publish=1')}
          ...
          style={{ background: 'var(--accent)', boxShadow: '0 8px 18px rgba(36,87,197,.24)' }}>
        ...
        {navItems.map(item => {
          ...
          return (
            <button key={item.path} onClick={() => navigate(item.path)}
              ...
              style={active
                ? { background: 'rgba(255,255,255,0.10)', color: 'var(--text-inverse)', fontWeight: 700, boxShadow: 'inset 3px 0 0 var(--accent)' }
                : { color: 'rgba(255,255,255,0.60)' }}>
              <div className="relative">
                <Icon size={18} />
                {item.badge > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 bg-[var(--error)] text-white text-[8px] font-bold px-1 rounded-full min-w-[14px] text-center leading-4">
      ...
      {upcomingEvents.length > 0 && (
        <div className="mx-3 mb-3 p-3 rounded-2xl" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.09)' }}>
          <div className="flex items-center gap-1.5 mb-2">
            <Calendar size={12} style={{ color: 'var(--accent-mist)' }} />
            <span className="text-[11px] font-semibold" style={{ color: 'var(--accent-mist)' }}>Próximos eventos</span>
          </div>
          {upcomingEvents.slice(0, 2).map(ev => (
            <div key={ev.id} className="text-[11px] leading-tight mb-1" style={{ color: 'rgba(255,255,255,0.70)' }}>
              <span className="font-medium" style={{ color: 'var(--text-inverse)' }}>{ev.title?.slice(0, 28)}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-3 pb-5 pt-2 border-t" style={{ borderColor: 'rgba(255,255,255,0.12)' }}>
        <div className="flex items-center gap-2.5 mb-2 px-1">
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
            style={{ background: 'var(--accent)', color: 'var(--text-inverse)', border: '1.5px solid rgba(255,255,255,0.2)' }}>
            {initials}
          </div>
          <span className="text-sm font-medium truncate" style={{ color: 'rgba(255,255,255,0.88)' }}>{name}</span>
        </div>
