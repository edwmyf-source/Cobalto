...
export const listPosts = async ({ cursor, limit = 20, filters = {}, sort = 'smart', userId } = {}) => {
  ...
  const buildSelect = (withEventDate) => `
      ...
      profiles!posts_author_id_fkey(id, full_name, identity_mode, identity_number, city, avatar_url, quimica_personaje, quimica_nombre, quimica_pts),
...
export const getPostById = async (id) => {
  const { data, error } = await supabase
    .from('posts')
    .select(`*, profiles!posts_author_id_fkey(id, full_name, identity_mode, identity_number, city, avatar_url, quimica_personaje, quimica_nombre, quimica_pts)`)
    .eq('id', id).single()
  if (error) throw error
  return data
}
